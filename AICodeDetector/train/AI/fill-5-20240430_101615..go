...
func GraphConvLayer(g *gorgonia.ExprGraph, input, adjacency *gorgonia.Node, outputDim int) (*gorgonia.Node, error) {
	inputDim := input.Shape()[1]
	weights := gorgonia.NewMatrix(g, tensor.Float32, gorgonia.WithShape("input", inputDim, outputDim), gorgonia.WithName("Weights"), gorgonia.WithInit(gorgonia.GlorotN(1.0)))
	
	// Perform the operation: (Adjacency * Input) * Weights
	// Note: This is a simplified version and may need adjustments based on your adjacency matrix adjInput, err := gorgonia.Mul(adjacency, input)
	if err != nil {
		return nil, err
	}
...
func main() {
	...
	// For example purposes, we assume a graph of 3 nodes and a feature vector of size 4 for each node
