...
func concatenateMultipleFiles(sources []string, dest string) {
	...
	for _, src := range sources {
		...
		if _, err := io.Copy(destFile, file); err != nil {
			log.Fatalf("failed to copy from source file %s: %s", src, err)
		}
		file.Close() // Close the source file after copying its content.
	}
}
...
func main() {
	...
	log.Println("Multiple files concatenated successfully.")
}
