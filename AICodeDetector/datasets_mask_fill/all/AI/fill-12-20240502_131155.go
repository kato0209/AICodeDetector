...
func NewLayer(inputSize, outputSize int) *Layer {
	...
	for i := range weights {
		...
		for j := range weights[i] {
			weights[i][j] = rand.Float64()*2 - 1 // Initialize with random values -1 and 1
		}
	}

	biases := make([]float64, outputSize)
	for i := range biases {
		biases[i] = rand.Float64()*2 - 1 // Initialize with random values -1 and 1
	}

	return &Layer{weights: weights, biases: biases}
}

func (l *Layer) RunNetwork(input []float64) []float64 {
