...
def load_base_model_and_tokenizer(args, model_config):
    ...
    if '13b' in name:
        base_model = transformers.AutoModelForCausalLM.from_pretrained(name, **base_model_kwargs, device_map="auto")
    elif 'codet5p' in name:
        base_model = transformers.AutoModelForSeq2SeqLM.from_pretrained(name, device_map="auto", torch_dtype=torch.float16,
        ...
        base_model = transformers.AutoModelForCausalLM.from_pretrained(name, **base_model_kwargs, trust_remote_code=True, torch_dtype=torch.float16)
        base_model.to(args.DEVICE)
    else:
        base_model = transformers.AutoModelForCausalLM.from_pretrained(name, **base_model_kwargs, trust_remote_code=True)
        base_model.to(args.DEVICE)
    ...
    if 'llama' in name.lower():
        base_tokenizer = transformers.LlamaTokenizer.from_pretrained(name, **optional_tok_kwargs)
    else:
